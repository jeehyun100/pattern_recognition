...
def load_datasets_by_label_for_test (te_data, grid_search=False, type='flatten'):#, activity_class="ALL", one_hot=False):
    """
    ...
    if 'flatten' in type:
        ...
        test_x = [x.flatten() for x in te_data_by_label[:,1]]
        test_y = te_data_by_label[:,2:3].astype(int)-1
    elif 'raw' in type:
        #train_x = np.array([x for x in tr_data_by_label[:,3]])
        #train_y = tr_data_by_label[:,0:1].astype(int)-1
        test_x =  np.array([x for x in te_data_by_label[:,1]])
        test_y = te_data_by_label[:,2:3].astype(int)-1
    elif 'fft' == type:
        #train_x_s = np.array([get_log_mel_transform(x) for x in tr_data_by_label[:,3]])
        #train_x = np.squeeze(train_x_s)
        #train_y = tr_data_by_label[:,0:1].astype(int)-1
        test_x_s =  np.array([get_log_mel_transform(x) for x in te_data_by_label[:,1]])
        ...
        #train_x = np.squeeze(train_x_s)
        #train_y = tr_data_by_label[:,0:1].astype(int)-1
        test_x_s =  np.array([get_log_mel_transform(x, "NN") for x in te_data_by_label[:,1]])
        test_x = np.squeeze(test_x_s)
        test_y = te_data_by_label[:,2:3].astype(int)-1
        #del train_x_s
        del test_x_s
        ...
        #train_x = np.squeeze(train_x_s)
        #train_y = tr_data_by_label[:,0:1].astype(int)-1
        test_x_s =  np.array([get_vq(x) for x in te_data_by_label[:,1]])
        test_x = np.squeeze(test_x_s)
        test_y = te_data_by_label[:,2:3].astype(int)-1
        #del train_x_s
        del test_x_s
        ...
        #train_x = np.squeeze(train_x_s)
        #train_y = tr_data_by_label[:,0:1].astype(int)-1
        test_x_s =  np.array([get_vq(x,"NN") for x in te_data_by_label[:,1]])
        test_x = np.squeeze(test_x_s)
        test_y = te_data_by_label[:,2:3].astype(int)-1
        #del train_x_s
        del test_x_s
...
def load_datasets_for_test(data_path, npz=None , shuffle= False):
    ...
    if npz == None:
        ...
        for file in glob.glob(data_path + "/*.txt"):
            ...
            col_list.append(os.path.basename(file))
            col_list.append(row)
        ...
        np.savez("./npz/activity_nparray3_test.npz", data = np_all_data)
        print("save complete")
    else:
        np_all_data = np.load("./npz/activity_nparray3_test.npz",allow_pickle=True)['data']
...
def predict_resNet(input_shape, model):
    # history = LossHistory()
...
def draw_plot(one_hot_predictions, y_test, te_data ):

    predictions = one_hot_predictions.argmax(1)+1
    np_file_name = te_data[:, 0].reshape((-1,1))
    np_predict = predictions.reshape((-1, 1))
    np_result = np.concatenate((np_file_name,np_predict), axis=1)

    pd.DataFrame(np_result).to_csv("./result_paikjeehyun.csv",index = False, header=False)
    #np.savetxt("result_paikjeehyun.csv", np_result, delimiter=",")
    print("")
...
if __name__ == "__main__":
    ...
    data_path = "./test_data"
    te_data = load_datasets_for_test(data_path, None, shuffle=True)
    X_test, y_test = load_datasets_by_label_for_test(te_data, type='nnvq')#nnvq, nnfft, raw
    ...
    predictions = predict_resNet(input_shape, new_model)
    draw_plot(predictions, y_test, te_data)
