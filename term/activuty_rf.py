...
def get_vq(p_data):
    """
        make initial parameters for vector quantization
    """
...
def rf_grid_search_train(tr_data, te_data):
...
def rf_train(tr_data, te_data):
    ...
    x_train, y_train, x_test, y_test = load_datasets_by_label(tr_data, te_data,  label=None, type='fft')
    params = {'criterion': 'entropy', 'max_depth': 20, 'max_features': 'auto', 'n_estimators': 400, 'verbose':2, 'n_jobs':8}
    accuracies = []
...
def exam_all_train(tr_data, te_data):
    ...
    feature_type= 'fft' #'fft', 'flatten, vq'
...
if __name__ == "__main__":
    ...
    data_path = "./mlpr20_project_data_new"
    ...
    #rf_grid_search_train(tr_data, te_data )
